...
router.get("/userinfo", async (req, res) => {
  ...
  let id = req.user.id;
  // 2、查询数据库
  let sql = "select id, username, nickname, email, user_pic from test where id = ?";
...
router.post("/updatepwd", async (req, res) => {
  ...
  // 2、操作数据库
  let sql = "update test set password = ? where id = ? and password = ?";
...
router.post("/update/avatar", async (req, res) => {
  ...
  console.log(params);
  // 2、操作数据库
  let sql = "update test set user_pic = ? where id = ?";
...
router.post("/userinfo", async (req, res) => {
  ...
  let params = req.body;
  // 2、操作数据库
  let sql = "update test set nickname = ?, email = ? where id = ?";
